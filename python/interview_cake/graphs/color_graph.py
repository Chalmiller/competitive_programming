...
def color_graph(graph, colors):
    ...
    """
    Task: Color the graph using at most D+1 colors
    Intuition: Breadth first search would be a good start here

    Algorith:
      - Traverse the graph via breadth first search
      - 
    """
    for node in graph:

      if node in node.neighbors:
        raise Exception('Legal coloring impossible for nbode with loop: {}'.format(node.label))
